-- ============================================================================
-- UPDATE: vw_creative_analysis_complete (v3.4) - INCLUDE ULTEC + FIX MISSING
-- Purpose: 1. Remove filter for '%Ultec%' campaigns (requested by user).
--          2. Include rows with NULL entity_id (Unknown Ads) to match Budget.
-- ============================================================================
...
WHERE p.platform = 'META'
  -- REMOVED: AND p.entity_id IS NOT NULL
  -- REMOVED: AND p.campaign_name NOT ILIKE '%Ultec%' (Requested inclusion)
